...
// Import LOA interaction router interceptor 
import { handleLOAButtons } from '../commands/moderation/loa.js';
        ...
        if (interaction.isChatInputCommand()) {
          ...
          if (autocompleteCommand?.autocomplete) {
            try {
              ...
              logger.error('Error handle command autocomplete:', {
                error: error.message,
                guildId: interaction.guildId,
                commandName: interaction.commandName
              });
          ...
          // INTERCEPT: Pass LOA review Accept/Deny buttons to handler immediately
          if (interaction.customId.startsWith('loa_')) {
            await handleLOAButtons(interaction).catch(() => {});
            return;
          }

          if (interaction.customId.startsWith('shared_todo_')) {
            const parts = interaction.customId.split('_');
            const buttonType = parts.slice(0, 3).
